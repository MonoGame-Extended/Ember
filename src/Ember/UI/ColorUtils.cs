...
public static class ColorUtils
{
    ...
    public static SysVec4 WithOpacity(SysVec4 color, float opacity)
    {
        return new SysVec4(color.X, color.Y, color.Z, Math.Clamp(opacity, 0.0f, 1.0f));
    }
    ...
    public static SysVec4 Darken(SysVec4 color, float factor = 0.8f)
    {
        factor = Math.Clamp(factor, 0.0f, 1.0f);
        return new SysVec4(color.X * factor, color.Y * factor, color.Z * factor, color.W);
    }
    ...
    public static SysVec4 Lighten(SysVec4 color, float factor = 1.2f)
    {
